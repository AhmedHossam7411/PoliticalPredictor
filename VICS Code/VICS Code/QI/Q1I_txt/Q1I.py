...
def main():
    ...
    words_folder = os.path.join(script_dir, 'Lists Q1I')

    # File paths
    positive_self_attributions_file = os.path.join(words_folder, 'positive_self_attributions.txt')
    negative_self_attributions_file = os.path.join(words_folder, 'negative_self_attributions.txt')
    input_text_file = os.path.join(base_path, 'input_text.txt')

    # Load words
    positive_words = load_words(positive_self_attributions_file)
    negative_words = load_words(negative_self_attributions_file)
    ...
    PP = positive_count / total_words if total_words else 0.0
    NP = negative_count / total_words if total_words else 0.0
    index = PP-NP
    # Output results
    print(f"Positive Self Attributions words count: {positive_count}")
    print(f"Negative Self Attributions count: {negative_count}")
    print(f"Strategy Index: {index:.2f}")
