...
def main():
    ...
    # File paths
    deeds_words_file = os.path.join(words_folder, 'deeds.txt')
    words_words_file = os.path.join(words_folder, 'words.txt')
    input_text_file = os.path.join(base_path, 'input_text.txt')

    # Load words
    deeds_words = load_words(deeds_words_file)
    words_words = load_words(words_words_file)
    ...
    deeds_count = count_words_in_text(text, deeds_words)
    words_count = count_words_in_text(text, words_words)

    # Calculate Nature of Political Universe Index
    total_words = deeds_count + words_count
    DP=deeds_count/total_words
    WP=words_count/total_words
    PD=abs(WP-DP)
    index = 1-PD


    # Output results
    print(f"Deeds count: {deeds_count}")
